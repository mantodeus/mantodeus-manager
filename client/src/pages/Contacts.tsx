        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredContacts.map((contact) => {
            ...
            return (
              <Card
                key={contact.id}
                className={`bg-gray-900 border-gray-700 p-4 hover:border-[#00ff88] transition-all ${
                  selectedIds.has(contact.id) ? "ring-2 ring-[#00ff88]" : ""
                }`}
              ...
              {contact.notes && (
                <p className="text-gray-500 text-xs mt-3 pt-3 border-t border-gray-700">{contact.notes}</p>
              )}
            </Card>
            );
          })}
