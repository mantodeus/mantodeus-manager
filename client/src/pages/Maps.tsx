...
export default function Maps() {
  ...
  const jobsIds = useMemo(() => jobs.map(j => j.id).join(','), [jobs]);
  ...
  useEffect(() => {
    ...
    // Search jobs
    jobs.forEach((job) => {
      const matchesName = job.title.toLowerCase().includes(query);
      const matchesLocation = job.location?.toLowerCase().includes(query);
      if ((matchesName || matchesLocation) && job.latitude && job.longitude) {
        results.push({
          id: `job-${job.id}`,
          name: job.title,
  ...
  }, [searchQuery, jobsIds, contactsIds, locationsIds, jobs, contacts, locations]);
    ...
    <div className="container mx-auto py-8 px-4">
      ...
      <div className="relative mb-6">
        <div className="relative">
          ...
          <Input
            type="text"
            placeholder="Search jobs, contacts, or locations..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            onFocus={() => searchResults.length > 0 && setShowSearchResults(true)}
            className="pl-10 pr-10 bg-gray-900 border-gray-700 focus:border-[#00ff88]"
        ...
        {showSearchResults && (
          <Card className="absolute z-50 w-full mt-2 bg-gray-900 border-gray-700 max-h-96 overflow-y-auto">
            {searchResults.length > 0 ? (
              ...
              <div className="p-2">
                ...
                <button
                  ...
                  className="w-full text-left p-3 hover:bg-gray-800 rounded-md transition-colors flex items-start gap-3 border border-dashed border-gray-700"
                >
                  <MapPin className="h-4 w-4 text-[#00ff88] mt-1 flex-shrink-0" />
