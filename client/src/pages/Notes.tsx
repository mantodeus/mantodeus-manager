...
export default function Notes() {
  ...
  // Using projects instead of jobs
  ...
  const filteredNotes = notes.filter((note) => {
    ...
    const matchesProject = filterProject === "all" || note.jobId?.toString() === filterProject;
  ...
  const getProjectName = (jobId: number | null) => {
          ...
          {filteredNotes.map((note) => {
            ...
            return (
              ...
              <div className="flex flex-col gap-1 text-xs text-muted-foreground">
                {getProjectName(note.jobId) && (
                  <div className="flex items-center gap-1">
                    <span className="text-[#00ff88]">Project:</span>
                    <span>{getProjectName(note.jobId)}</span>
                  </div>
                )}
