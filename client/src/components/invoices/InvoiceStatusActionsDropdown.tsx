...
import { Loader2, Send, CheckCircle2, X, FileX, RotateCcw, Plus, XCircle, CurrencyEuro } from "@/components/ui/Icon";
...
export function InvoiceStatusActionsDropdown({
  ...
  const renderStatusBadge = () => {
    ...
    if (invoiceState === "SENT") {
      return (
        <Button
          ...
          variant="outline"
          size="default"
          className="text-sm font-semibold border-[var(--state-info)]/50 text-[var(--state-info)] hover:bg-[var(--state-info)]/10 hover:border-[var(--state-info)]/70 cursor-pointer"
        >
          SENT
        </Button>
      );
    ...
    if (invoiceState === "DRAFT") {
      return <Button asChild={false} variant="outline" size="default" className="text-sm font-semibold border-yellow-500 text-yellow-600 dark:text-yellow-400 dark:border-yellow-600 cursor-pointer">DRAFT</Button>;
    }
