...
if [ -z "$CURRENT_REMOTE" ]; then
  echo "�?O Git remote 'origin' not set. Please configure it before deploying."
  exit 1
fi
if echo "$CURRENT_REMOTE" | grep -qE "^https?://"; then
  echo "�?O Git remote uses HTTPS, which will prompt for credentials in production."
...
if ! echo "$CURRENT_REMOTE" | grep -qE "git@github.com:mantodeus/mantodeus-manager.git|ssh://git@github.com/mantodeus/mantodeus-manager.git"; then
  echo "�?O Git remote is not the expected SSH URL for mantodeus/mantodeus-manager."
  echo "   Current: $CURRENT_REMOTE"
  exit 1
fi
echo "�o. Git remote: ${CURRENT_REMOTE}"
...
if [ ! -d "./drizzle" ]; then
  echo "�?O drizzle/ folder not found. Cannot apply migrations."
  exit 1
fi
MIGRATION_COUNT=$(ls -1 ./drizzle/*.sql 2>/dev/null | wc -l | tr -d ' ')
if [ "$MIGRATION_COUNT" = "0" ]; then
  echo "�?O No migration files found in drizzle/. Aborting deploy."
  exit 1
fi
if [ ! -f "./drizzle.config.ts" ]; then
  echo "�?O drizzle.config.ts not found. Aborting deploy."
  exit 1
fi
...
if [ $MIGRATE_STATUS -ne 0 ]; then
  echo "�?O Migration command failed"
  exit 1
fi
if echo "$MIGRATE_OUTPUT" | grep -qiE "none found|failed"; then
  echo "�?O Migration output indicates missing or failed migrations"
  exit 1
fi

# Post-migrate sanity check
if [ ! -f "./scripts/check-migration-columns.js" ]; then
  echo "�?O scripts/check-migration-columns.js not found. Aborting deploy."
  exit 1
fi
